...
const onBlur = () => {
  ...
  if (!state.isInvalid) {
    ...
    emit(
      "update:modelValue",
      toDate(Date.parse(`${state.date} ${state.formattedTime}`))
    );
  }
};
...
<template>
  <div
    class="relative flex w-full py-1 transition border-2 border-transparent divide-x rounded-md"
    ref="dateTimeField"
    :class="{
      'border-red-400 bg-red-50': isInvalid,
      'border-gray-400': isSelected,
    }"
  >
    <div
      class="absolute top-0 px-2 py-1 text-sm text-white bg-black rounded-md hover:opacity-100 opacity-60 -right-24"
      role="error"
      v-if="isInvalid"
    >
      Invalid date
    ...
    <input
      v-model="date"
      :time="includeTime"
      class="w-full px-2 text-sm bg-transparent focus:outline-none"
    ...
    <input
      ...
      :time="includeTime"
      @blur="onBlur"
      class="w-full px-2 text-sm focus:outline-none"
      placeholder="hh:mm a"
      ref="timeInput"
      @click="onFocus"
    />
