<template>
    <nav class="flex items-center h-24 mx-auto max-w-7xl">
        <div class="z-50 flex flex-wrap items-center justify-between w-full h-24 mx-auto font-medium">
            <component :is="routerComponent" to="/" class="py-4 pr-4 cursor-pointer">
            ...
            <div class="flex-col items-center justify-end hidden w-full h-full mt-12 font-bold text-center md:flex md:w-2/3 md:mt-0 md:flex-row md:items-center">
                ...
                <div class="md:ml-8">
                    <component :is="routerComponent" :to="action.url" :class="action.class" class="w-full px-4 py-2 font-bold transition rounded-md cursor-pointer md:mr-3 lg:mr-3 md:w-auto" v-for="action in actions">
                    {{ action.name }}
                    </component>
                </div>
            </div>
